...
public class Clinic {
    ...
    public String getPetByClient(String clientName) {
        if (clientName == null) return "Такого клиента не существует";/*
    ...
    public Client findClientsByPetName(final String petName) {
        if (petName == null) return new Client(null, null);
        for (Client client : clients) {
    ...
    public void deleteClient(String nameClientOrNamePet) {
        for (Client client : clients) {
            if (nameClientOrNamePet.equalsIgnoreCase(client.getName()) ||
                    nameClientOrNamePet.equalsIgnoreCase(client.getPet().getName())) {
                client = null;
            }
        }
    }
