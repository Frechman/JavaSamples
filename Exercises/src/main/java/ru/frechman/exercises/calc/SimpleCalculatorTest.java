...
public class SimpleCalculatorTest {
    ...
    public void testIntDiv() {
        int actual = SimpleCalculator.div(88, -7);
        int expected = -12;
        Assert.assertEquals("88 / -7 is " + expected, expected, actual);
    }

    public void testDoubleDiv() {
        double actual = SimpleCalculator.div(45.6, 1.36);
    ...
    public void testIntDivToDivideByZero() {
        ...
        try {
            SimpleCalculator.div(45, 0);
        } catch (ArithmeticException ae) {
            String actual = ae.getMessage();
            Assert.assertEquals("45 / 0 is \"" + expected + "\"", expected, actual);
            return;
        }
        System.out.println("45 / 0 is \"" + expected + "\"" + " - Test failed!");
    }

    public void testDoubleDivToDivideByZero() {
        String expected = "Divide by zero!";
        try {
            SimpleCalculator.div(75.6, 0.0);
        } catch (ArithmeticException ae) {
            String actual = ae.getMessage();
            Assert.assertEquals("75.6 / 0.0 is \"" + expected + "\"", expected, actual);
            return;
        }
        System.out.println("75.6 / 0.0 is \"" + expected + "\"" + " - Test failed!");
    }
}